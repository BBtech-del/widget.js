function buildLeadModal(onSubmit) {
  ...
  const title = create("div", { innerText: "👋 Welcome! Let’s get to know each other." }, {
    fontSize: "18px", fontWeight: "600", marginBottom: "10px"
  });

  const desc = create("div", { innerText: "Before we start chatting, could you share your name and email? This helps me personalize our conversation." }, {
    fontSize: "14px", opacity: "0.9", marginBottom: "16px"
  });

  const form = create("form");
  const nameInput = create("input", { type: "text", placeholder: "Your name", required: true }, {
    width: "100%", padding: "10px 12px", borderRadius: "8px", border: "1px solid #ddd",
    marginBottom: "12px", outline: "none"
  });

  const emailInput = create("input", { type: "email", placeholder: "you@example.com", required: true }, {
    width: "100%", padding: "10px 12px", borderRadius: "8px", border: "1px solid #ddd",
    marginBottom: "12px", outline: "none"
  });

  const err = create("div", { innerText: "" }, {
    color: "#c0392b", fontSize: "12px", minHeight: "16px", marginBottom: "8px"
  });

  const submitBtn = create("button", { type: "submit", innerText: "Start Chat" }, {
  ...
  form.addEventListener("submit", e => {
    ...
    overlay.remove();
    onSubmit({ name: nameInput.value.trim(), email: emailInput.value.trim() });
  });
}
